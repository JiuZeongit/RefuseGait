import torch
import torch.nn as nn
import torch.utils.checkpoint
from einops import rearrange
from ...modules import SetBlockWrapper, SeparateFCs, SeparateBNNecks, PackSequenceWrapper, HorizontalPoolingPyramid
from torch.nn import functional as F
from ...backbones.resnet import ResNet9
import os
import sys
# ######################################## GaitBase ###########################################

def conv1x1(in_planes, out_planes, stride=1):
    """1x1 convolution"""
    return nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)

def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    """3x3 convolution with padding"""
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=dilation, groups=groups, bias=False, dilation=dilation)

class AttentionFusion(nn.Module): 
    def __init__(self, in_channels, squeeze_ratio, feat_len):
        super(AttentionFusion, self).__init__()
        hidden_dim = int(in_channels / squeeze_ratio)
        self.feat_len = feat_len
        self.conv = SetBlockWrapper(
            nn.Sequential(
                conv1x1(in_channels * feat_len, hidden_dim), 
                nn.BatchNorm2d(hidden_dim), 
                nn.ReLU(inplace=True), 
                conv3x3(hidden_dim, hidden_dim), 
                nn.BatchNorm2d(hidden_dim), 
                nn.ReLU(inplace=True), 
                conv1x1(hidden_dim, in_channels * feat_len), 
            )
        )
    
    def forward(self, feat_list): 
        '''
            sil_feat: [n, c, s, h, w]
            map_feat: [n, c, s, h, w]
            ...
        '''
        feats = torch.cat(feat_list, dim=1)
        score = self.conv(feats) # [n, 2 * c, s, h, w]
        score = rearrange(score, 'n (c d) s h w -> n c d s h w', d=self.feat_len)
        score = F.softmax(score, dim=2)
        retun = feat_list[0]*score[:,:,0]
        for i in range(1, self.feat_len):
            retun += feat_list[i]*score[:,:,i]
        return retun


from torchvision.models.resnet import BasicBlock, Bottleneck, ResNet
from ...modules import BasicConv2d
block_map = {'BasicBlock': BasicBlock,
             'Bottleneck': Bottleneck,
             }

class Pre_ResNet9(ResNet):
    def __init__(self, type, block, channels=[32, 64, 128, 256], in_channel=1, layers=[1, 2, 2, 1], strides=[1, 2, 2, 1], maxpool=True):
        if block in block_map.keys():
            block = block_map[block]
        else:
            raise ValueError(
                "Error type for -block-Cfg-, supported: 'BasicBlock' or 'Bottleneck'.")
        self.maxpool_flag = maxpool
        super(Pre_ResNet9, self).__init__(block, layers)

        # Not used #
        self.fc = None
        self.layer2 = None
        self.layer3 = None
        self.layer4 = None
        ############
        self.inplanes = channels[0]
        self.bn1 = nn.BatchNorm2d(self.inplanes)

        self.conv1 = BasicConv2d(in_channel, self.inplanes, 3, 1, 1)

        self.layer1 = self._make_layer(
            block, channels[0], layers[0], stride=strides[0], dilate=False)

    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        if blocks >= 1:
            layer = super()._make_layer(block, planes, blocks, stride=stride, dilate=dilate)
        else:
            def layer(x): return x
        return layer

    def forward(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        if self.maxpool_flag:
            x = self.maxpool(x)

        x = self.layer1(x)
        return x

class Post_ResNet9(ResNet):
    def __init__(self, type, block, channels=[32, 64, 128, 256], in_channel=1, layers=[1, 2, 2, 1], strides=[1, 2, 2, 1], maxpool=True):
        if block in block_map.keys():
            block = block_map[block]
        else:
            raise ValueError(
                "Error type for -block-Cfg-, supported: 'BasicBlock' or 'Bottleneck'.")
        super(Post_ResNet9, self).__init__(block, layers)
        # Not used #
        self.fc = None
        self.conv1 = None
        self.bn1 = None
        self.relu = None
        self.layer1 = None
        ############
        self.inplanes = channels[0]
        self.layer2 = self._make_layer(
            block, channels[1], layers[1], stride=strides[1], dilate=False)
        self.layer3 = self._make_layer(
            block, channels[2], layers[2], stride=strides[2], dilate=False)
        self.layer4 = self._make_layer(
            block, channels[3], layers[3], stride=strides[3], dilate=False)

    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        if blocks >= 1:
            layer = super()._make_layer(block, planes, blocks, stride=stride, dilate=dilate)
        else:
            def layer(x): return x
        return layer

    def forward(self, x):
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)
        return x


from opengait.utils import get_valid_args, is_list, is_dict, np2var, ts2np, list2var, get_attr_from
from ... import backbones



class PhaseCanonicalPooling(nn.Module):
    """
    Spectrum-guided phase canonical pooling.

    Input:
        x: [N, C, T, H, W]

    Output:
        a one-element tuple containing [N, C, H, W],
        matching the interface of torch.max used by
        PackSequenceWrapper.
    """

    def __init__(
        self,
        in_channels,
        num_phases=8,
        max_frequency_bin=4,
        spectrum_temperature=0.25,
        phase_concentration=4.0,
        mix_init=0.5,
    ):
        super().__init__()

        self.in_channels = int(in_channels)
        self.num_phases = int(num_phases)
        self.max_frequency_bin = int(max_frequency_bin)
        self.spectrum_temperature = float(
            spectrum_temperature
        )
        self.phase_concentration = float(
            phase_concentration
        )

        if self.num_phases < 2:
            raise ValueError("num_phases must be >= 2")

        if self.max_frequency_bin < 1:
            raise ValueError(
                "max_frequency_bin must be >= 1"
            )

        # Each channel independently learns how to combine
        # the K canonical phase responses.
        self.phase_reduce = nn.Conv2d(
            self.in_channels * self.num_phases,
            self.in_channels,
            kernel_size=1,
            groups=self.in_channels,
            bias=True,
        )

        nn.init.constant_(
            self.phase_reduce.weight,
            1.0 / self.num_phases,
        )
        nn.init.zeros_(self.phase_reduce.bias)

        mix_init = min(max(float(mix_init), 0.01), 0.99)
        mix_tensor = torch.tensor(mix_init)
        self.mix_logit = nn.Parameter(
            torch.log(mix_tensor / (1.0 - mix_tensor))
        )

    def _estimate_phase(self, x):
        """
        Estimate a differentiable dominant motion frequency
        and phase from the temporal feature-energy curve.

        Args:
            x: [N, C, T, H, W]

        Returns:
            phase: [N, T], in radians.
        """
        n, _, t, _, _ = x.shape

        # Use float32 FFT even under mixed-precision evaluation.
        signal = x.detach().float().abs().mean(
            dim=(1, 3, 4)
        )

        signal = signal - signal.mean(
            dim=1,
            keepdim=True,
        )

        window = torch.hann_window(
            t,
            periodic=False,
            dtype=signal.dtype,
            device=signal.device,
        ).unsqueeze(0)

        spectrum = torch.fft.rfft(
            signal * window,
            dim=1,
        )

        available_bins = spectrum.shape[1] - 1
        max_bin = min(
            self.max_frequency_bin,
            available_bins,
        )

        if max_bin < 1:
            return torch.zeros(
                n,
                t,
                dtype=signal.dtype,
                device=signal.device,
            )

        candidate_spectrum = spectrum[:, 1:max_bin + 1]
        power = candidate_spectrum.abs().square()

        # Normalization makes temperature less sensitive
        # to feature magnitude.
        power = power / (
            power.mean(dim=1, keepdim=True) + 1e-6
        )

        frequency_probability = F.softmax(
            power / self.spectrum_temperature,
            dim=1,
        )

        frequency_bins = torch.arange(
            1,
            max_bin + 1,
            dtype=signal.dtype,
            device=signal.device,
        )

        dominant_frequency = (
            frequency_probability * frequency_bins.unsqueeze(0)
        ).sum(dim=1)

        dominant_coefficient = (
            frequency_probability.to(
                candidate_spectrum.dtype
            )
            * candidate_spectrum
        ).sum(dim=1)

        initial_phase = torch.angle(
            dominant_coefficient
        )

        time_index = torch.arange(
            t,
            dtype=signal.dtype,
            device=signal.device,
        ).unsqueeze(0)

        phase = (
            2.0
            * torch.pi
            * dominant_frequency.unsqueeze(1)
            * time_index
            / max(t, 1)
            + initial_phase.unsqueeze(1)
        )

        return phase

    def _phase_pool(self, x):
        n, c, t, h, w = x.shape

        if c != self.in_channels:
            raise ValueError(
                f"Expected {self.in_channels} channels, "
                f"but received {c}"
            )

        phase = self._estimate_phase(x)

        phase_centers = (
            2.0
            * torch.pi
            * torch.arange(
                self.num_phases,
                dtype=phase.dtype,
                device=phase.device,
            )
            / self.num_phases
        )

        # [N, K, T]
        assignment_logits = (
            self.phase_concentration
            * torch.cos(
                phase.unsqueeze(1)
                - phase_centers.view(1, -1, 1)
            )
        )

        assignment = F.softmax(
            assignment_logits,
            dim=-1,
        ).to(dtype=x.dtype)

        # [N, C, K, H, W]
        canonical_phase_features = torch.einsum(
            "nkt,ncthw->nckhw",
            assignment,
            x,
        )

        canonical_phase_features = (
            canonical_phase_features
            .contiguous()
            .view(
                n,
                c * self.num_phases,
                h,
                w,
            )
        )

        phase_feature = self.phase_reduce(
            canonical_phase_features
        )

        max_feature = torch.max(
            x,
            dim=2,
        )[0]

        mix = torch.sigmoid(
            self.mix_logit
        ).to(dtype=x.dtype)

        output = (
            (1.0 - mix) * max_feature
            + mix * phase_feature
        )

        return output

    def forward(self, x, dim=2):
        if dim != 2:
            x = x.movedim(dim, 2)

        pooled = self._phase_pool(x)

        # Keep compatibility with the original:
        # self.TP(...)[0]
        return (pooled,)


class Baseline1(nn.Module):
    def __init__(self, model_cfg):
        super(Baseline1, self).__init__()
        model_cfg['backbone_cfg']['in_channel'] = model_cfg['Action_Branch']['target_dim']
        # model_cfg['backbone_cfg']['in_channel'] = 6
        self.pre_act = SetBlockWrapper(Pre_ResNet9(**model_cfg['backbone_cfg']))

        model_cfg['backbone_cfg']['in_channel'] = model_cfg['Appearance_Branch']['target_dim']
        self.pre_app = SetBlockWrapper(Pre_ResNet9(**model_cfg['backbone_cfg']))

        self.post_backbone = SetBlockWrapper(Post_ResNet9(**model_cfg['backbone_cfg']))
        self.FCs = SeparateFCs(**model_cfg['SeparateFCs'])
        self.BNNecks = SeparateBNNecks(**model_cfg['SeparateBNNecks'])
        temporal_cfg = model_cfg.get(
            "TemporalPooling",
            {"type": "max"},
        )
        temporal_type = temporal_cfg.get(
            "type",
            "max",
        ).lower()

        if temporal_type == "max":
            self.TP = PackSequenceWrapper(torch.max)
        elif temporal_type == "phase":
            self.TP = PackSequenceWrapper(
                PhaseCanonicalPooling(
                    in_channels=model_cfg[
                        "backbone_cfg"
                    ]["channels"][-1],
                    num_phases=temporal_cfg.get(
                        "num_phases",
                        8,
                    ),
                    max_frequency_bin=temporal_cfg.get(
                        "max_frequency_bin",
                        4,
                    ),
                    spectrum_temperature=temporal_cfg.get(
                        "spectrum_temperature",
                        0.25,
                    ),
                    phase_concentration=temporal_cfg.get(
                        "phase_concentration",
                        4.0,
                    ),
                    mix_init=temporal_cfg.get(
                        "mix_init",
                        0.5,
                    ),
                )
            )
        else:
            raise ValueError(
                "Unknown TemporalPooling type: "
                f"{temporal_type}"
            )
        self.HPP = HorizontalPoolingPyramid(bin_num=model_cfg['bin_num'])

        self.fusion = AttentionFusion(**model_cfg['AttentionFusion'])

    def get_backbone(self, backbone_cfg):
        """Get the backbone of the model."""
        if is_dict(backbone_cfg):
            Backbone = get_attr_from([backbones], backbone_cfg['type'])
            valid_args = get_valid_args(Backbone, backbone_cfg, ['type'])
            return Backbone(**valid_args)
        if is_list(backbone_cfg):
            Backbone = nn.ModuleList([self.get_backbone(cfg)
                                      for cfg in backbone_cfg])
            return Backbone
        raise ValueError(
            "Error type for -Backbone-Cfg-, supported: (A list of) dict.")

    def vis_forward(self, appearance, action, seqL):
        action = self.pre_act(action)  # [n, c, s, h, w]
        appearance = self.pre_app(appearance)  # [n, c, s, h, w]
        outs = self.fusion([action, appearance])
        return action, appearance, outs

    def forward(self, appearance, action, seqL):
        action = self.pre_act(action)  # [n, c, s, h, w]
        appearance = self.pre_app(appearance)  # [n, c, s, h, w]
        outs = self.fusion([action, appearance])
        # heat_mapt = rearrange(outs, 'n c s h w -> n s h w c')
        del action, appearance
        outs = self.post_backbone(outs)

        # Temporal Pooling, TP
        outs = self.TP(outs, seqL, options={"dim": 2})[0]  # [n, c, h, w]

        # Horizontal Pooling Matching, HPM
        outs = self.HPP(outs)  # [n, c, p]

        embed_1 = self.FCs(outs)  # [n, c, p]
        _, logits = self.BNNecks(embed_1)  # [n, c, p]
        # return embed_1, logits, heat_mapt
        return embed_1, logits
